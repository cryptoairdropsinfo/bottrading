...
def fetch_klines(symbol: str, interval: str, limit: int = 200) -> pd.DataFrame | None:
    ...
    - Devuelve orden DESCENDENTE → invertimos con reversed()
    ...
    # Bybit devuelve descendente → invertir para orden cronológico
...
def get_entry_signal(df_15m: pd.DataFrame, direction_30m: str | None) -> str | None:
    ...
    trend_ok_long = direction_30m in ("LONG", None)
...
def main():
    print("=" * 62)
    print("  SCALPING BOT — Reversión a la Media  |  Bybit Futures")
    print(f"  TF: {TF_ENTRY}m entrada | {TF_FILTER}m filtro")
